...
export class InterviewsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.addInterviewForm = this.fb.group({
      ...
      'notes': [this.notes, []],
      'interviewFiles': [this.interviewFiles, []],
    });

    // get all user interviews to display
    this.getJobs();
  }

  onAddSubmit() {
    // validate form
    var validated = this.validateAddForm(this.addInterviewForm);
    ...
    if(validated.status) {
      ...
      this.manage.addJob(
        this.addInterviewForm.get('companyName').value,
        this.addInterviewForm.get('jobTitle').value,
        this.addInterviewForm.get('link').value,
        this.addInterviewForm.get('notes').value,
        JOB_TYPE_NAME,
        temp,
        this.user.user_id
      ).subscribe(data => {
